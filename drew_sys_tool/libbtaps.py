...
"""
This was updated very quickly to work with python 3. done quickly and still
probably has many bugs. main changes:
    -everywhere it was buffer(array(...)) was changed to bytes(array(...))
    -stripped out the b' from the serial_port_profile in the connect funtion of BTaps class
...
class BTaps:
    ...
    def connect(self):
        ...
        serial_port_profile = (binascii.a2b_qp(bluetooth.SERIAL_PORT_PROFILE[0]), bluetooth.SERIAL_PORT_PROFILE[1])
        # strip out the b' from the serial_port_profile

        for match in matches:
            if match['protocol'] == 'RFCOMM' and serial_port_profile in match['profiles']:
                device = match
##                print("found a match")
...
##            print(match['profiles'])
##            print(match['profiles'][0])
##            blah = match['profiles'][0][0]
##            print(blah)
##            print((blah, match['profiles'][0][1]))
##            print(binascii.a2b_qp(bluetooth.SERIAL_PORT_PROFILE[0]))
##            print(bluetooth.SERIAL_PORT_PROFILE[0])
##            blah2 = (binascii.a2b_qp(bluetooth.SERIAL_PORT_PROFILE[0]), bluetooth.SERIAL_PORT_PROFILE[1])
##            print (blah2)
##            print (blah2 in match['profiles'])
##            print(bluetooth.SERIAL_PORT_PROFILE in match['profiles'])
            return 0
    ...
    def set_datetime_now(self):
        ...
        # used to be buffer(array('B', ..)) no longer supported in python 3
        payload = array('B', [0xcc, 0xaa, 0x09, 0x09, 0x01,
    ...
    def get_dev_name(self):
        ...
        # used to be buffer(array(...))
        payload = bytes(array('B', [0xCC, 0xAA, 0x03, 0x18, 0x01, 0x19]))
        self.socket.send(payload)
        response = self.__recv_data()

        # this strips out trailing \x00 from the name
        name = str(response[5:])
    ...
    def set_dev_name(self, name):
        ...
        name_array = array('B', bytes(name, encoding='ascii'))
    ...
    def set_switch(self, on):
        if on:
            ...
            payload = bytes(array('B', [0xCC, 0xAA, 0x03, 0x01, 0x01, 0x01]))
        else:
            # OFF Payload = 0xCCAA03010100
            payload = bytes(array('B', [0xCC, 0xAA, 0x03, 0x01, 0x01, 0x00]))
    ...
    def __set_timer(self, btapstimer, create=True):
        name_array = array('B', bytes(btapstimer.name, encoding='ascii'))
    ...
    def delete_timer(self, timer):
        ...
        payload = bytes(array('B', [0xCC, 0xAA, 0x04, 0x19, 0x01, 0x01, timer_id]))
        self.socket.send(payload)
